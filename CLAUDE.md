...
- `Despesa`/`DespesaRecorrente` do not yet have FK columns back to `Contrato`, `FolhaPagamento`, or `PrestacaoContas`, even though `OrigemDespesa` already has `CONTRATO`/`FOLHA`/`PRESTACAO_CONTAS` values — those target entities don't exist yet (readme sections 6-8). Add the FKs once those entities are modeled.
- Cross-cutting concerns from readme section 14 (soft delete via `@MappedSuperclass`, audit trail/Envers, multi-tenant isolation by `EmpresaPropria`) are documented but not yet implemented anywhere in the model.
